...
BookList::BookList(const std::size_t& newSize)
    //initialize _bookArray with new dynamic array with size newSize
    //initialize _books_array_size and _capacity with newSize
...
BookList& BookList::operator+=(const BookList& rhs)
{
    ...
    //temporary variable for number of books in rhs
    std::size_t _len_rhs = rhs._books_array_size;
    for (std::size_t i = 0; i < _len_rhs; i++)
    {
        if (_books_array_size < _capacity)
        {
            //concatenate 1 book from rhs to lhs
            _bookArray[_books_array_size] = rhs._bookArray[i];
            //update number of books in the list
            ++_books_array_size;
        }
    }
    return *this;
...
BookList::~BookList()
{
    ...
    delete[] _bookArray;
    //null the pointer to avoid dangling pointer
    _bookArray = nullptr;
    //zero out number of books in the list
    //and maximum number of books in the dynamic array
    _books_array_size = 0;
    _capacity = 0;
}
